...
import utils.queue import q_receiver, q_cmsg_receiver
...
def recv_command():
    while True:
        key_command_msg = q_cmsg_receiver(key_command)
        if key_command_msg:
...
def recv_wall_distance():
    ...
    while True:
        calc_wall_distance_msg = q_receiver(calc_wall_distance)
...
def recv_obstacle():
    while True:
        obstacle_msg = int(q_receiver(obstacle))
        # print(obstacle_msg)
        if obstacle_msg:
